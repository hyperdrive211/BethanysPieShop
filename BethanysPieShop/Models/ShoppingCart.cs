...
namespace BethanysPieShop.Models
{
    public class ShoppingCart
    {
        ...
        {
            var shoppingCatItem = _appDbContext.ShoppingCartItems.SingleOrDefault(
                s => s.Pie.PieId == pie.PieId && s.ShoppingCartId == ShoppingCartId);

            var localAmount = 0;

            if (shoppingCatItem != null) {
                if (shoppingCatItem.Amount > 1)
                {
                    shoppingCatItem.Amount--;
                    localAmount = shoppingCatItem.Amount;
                }
                else
                {
                    _appDbContext.ShoppingCartItems.Remove(shoppingCatItem); 
                }
            }
            _appDbContext.SaveChanges();
